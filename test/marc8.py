...
class MARC8Test(TestCase):
    ...
    def test_marc8_reader_to_unicode(self):
        reader = MARCReader(open('test/marc8.dat'), to_unicode=True)
        r =  next(reader)
        self.assertEquals(type(r), Record)
        utitle = r['240']['a']
        self.assertEquals(type(utitle), str)
    ...
    def test_marc8_reader_to_unicode_bad_escape(self):
        ...
        self.assertEquals(type(r), Record)
        upublisher = r['260']['b']
        self.assertEquals(type(upublisher), str)
        self.assertEquals(upublisher, 'La Soci\xe9t\x1b,')

    def test_marc8_to_unicode(self):
        marc8_file = open('test/test_marc8.txt')
        utf8_file = open('test/test_utf8.txt')
        count = 0
    ...
    def test_writing_unicode(self):
        record = Record()
##        new_field = Field(245, ['1', '0'])
##        new_field.add_subfield('a',"\u1234".encode("cp1252"))
##        print(str(new_field['a']))
        record.add_field(Field(245, ['1', '0'], ['a', "\u1234".encode("cp1252")]))
        record.leader = '         a              '
        ...
        record = next(reader)
        print(record['245'])
        self.assertEqual(record['245']['a'], "\u1234".encode("cp1252"))
    ...
    def test_reading_utf8_with_flag(self):
        ...
        utitle = record['240']['a']
        self.assertEquals(type(utitle), str)
        self.assertEquals(utitle, 'De la solitude a' + chr(0x0300) +
            ' la communaute' + chr(0x0301) + '.')
    ...
    def test_subscript_2(self):
        self.assertEqual(marc8_to_unicode('CO\x1bb2\x1bs is a gas'), 'CO\u2082 is a gas')
        self.assertEqual(marc8_to_unicode('CO\x1bb2\x1bs'), 'CO\u2082')
